...
const escapeHTML = (() => {
  ...
  const regexString = `[${Object.keys(escapeChars).join('')}]`;
   ...
   * @param {string} str
   */
  return str => str.replace(regex, m => `&${escapeChars[m]};`);
})();
...
const gsUtils = window.gsUtils = {
  ...
  generateSuspendedUrl: function(url, title, scrollPos) {
    const args = `#ttl=${gsUtils.encodeString(title)}&pos=${gsUtils.encodeString(scrollPos || '0')}&uri=${gsUtils.encodeString(url)}`;
   ...
   * @returns {string}
   */
  getSuspendedBuggyTimeParam: function(urlStr) {
    const searchParams = new URLSearchParams(urlStr.slice(urlStr.indexOf('#') + 1));
    return searchParams.get('t', urlStr) || '';
  },
