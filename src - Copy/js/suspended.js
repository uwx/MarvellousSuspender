 ...
 * @param {{quickInit: boolean}} param2
 * @returns
 */
async function initTab(tab, { quickInit }) {
  const suspendedUrl = tab.url;

  // Set sessionId for subsequent checks
  document.sessionId = gsSession.getSessionId();
  ...
  setFaviconMeta(document, faviconMeta);
  ...
  // Add event listeners
  setUnloadTabHandler(window, tab);
  setUnsuspendTabHandlers(document, tab);
  ...
  setTheme(document, theme, isLowContrastFavicon);

  // Set command
  const suspensionToggleHotkey = await tgs.getSuspensionToggleHotkey();
  setCommand(document, suspensionToggleHotkey);

  // Set url
  setUrl(document, originalUrl);
  ...
  setReason(document, suspendReason);
  ...
  setScrollPosition(document, scrollPosition, previewMode);
...
function loadToastTemplate(_document) {
  ...
  _document.querySelectorAll('body')[0].append(toastEl);
}

function cleanUrl(urlStr) {
  // remove scheme
  if (urlStr.indexOf('//') > 0) {
    urlStr = urlStr.slice(Math.max(0, urlStr.indexOf('//') + 2));
  }
  // remove query string
  let match = urlStr.match(/\/?[?#]+/);
  if (match) {
    urlStr = urlStr.slice(0, Math.max(0, match.index));
  }
